/*
 * Copyright 2013-2019 Embedded Solutions
 *
...
#define TRIG_LOWER_THEN         (0)
#define TRIG_GREATER_SLOPE      (1)
...
#define ENC_DIR_COUNT           (1)
#define ENC_UP_COUNT            (2)
#define ENC_DOWN_COUNT          (3) 
...
EXTERNC MDAQ_API int mlink_hw_serial_no(int *link_fd, char *sn, int len);
...
EXTERNC MDAQ_API int mlink_dio_write( int *link_fd, uint8_t *dio, uint8_t *value, uint8_t count);
EXTERNC MDAQ_API int mlink_dio_read( int *link_fd, uint8_t *dio, uint8_t *value, uint8_t count);
...
EXTERNC MDAQ_API int mlink_enc_init( int *link_fd, uint8_t ch, uint8_t mode, int32_t init_value );
...
EXTERNC MDAQ_API int mlink_ai_scan_wait_until_done(int *link_fd, int timeout);
...
EXTERNC MDAQ_API int mlink_ai_scan_sync(int *link_fd, uint8_t dio, uint8_t edge);
